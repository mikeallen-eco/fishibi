...
f <- read.table("data/merged.uni.c10.l140.L190.cln.crabtag.ann.srt.tsv", 
                sep="\t", header=TRUE) %>%
...
fs2 <- fs %>%
  filter(order %notin% c("Primates", "Rodentia", "Anseriformes", 
         ...
         !is.na(order))
sum(select(fs2, starts_with("eDNA"))) # 7520118
...
# sum the reads across all negatives
sum(select(fn2, starts_with("ENC"), starts_with("FNC"), starts_with("PCR"), ends_with("-B"))) # 26037
